...
GROUP BY p.id";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        echo json_encode(['status' => 'error', 'message' => 'Product not found. No product with this ID.']);
    }
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $new_sku = filter_input(INPUT_POST, 'sku', FILTER_SANITIZE_STRING);
        $title = filter_input(INPUT_POST, 'product_name', FILTER_SANITIZE_STRING);
        ...
        if (!$id || !$new_sku || !$title || $price === false) {
        ...
        header('Location: ../index.html?message=Product updated successfully');
        ...
        header('Location: ../index.html?message=Error updating product');
    }
} else {
